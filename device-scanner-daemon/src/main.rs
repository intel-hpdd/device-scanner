...
fn update_udev(state: &mut State, cmd: UdevCommand) {
    match cmd {
        UdevCommand::Add(x) | UdevCommand::Change(x) => state.uevents.insert(x.devpath.clone(), x),
        UdevCommand::Remove(x) => state.uevents.remove(&x.devpath),
    };
}

fn update_mount(state: &mut State, cmd: MountCommand) {
    match cmd {
        MountCommand::AddMount(target, source, fstype, opts) => state
            .local_mounts
            .insert(Mount::new(target, source, fstype, opts)),
        MountCommand::RemoveMount(target, source, fstype, opts) => state
            .local_mounts
            .remove(&Mount::new(target, source, fstype, opts)),
        MountCommand::ReplaceMount(target, source, fstype, opts, old_ops) => {
            state.local_mounts.remove(&Mount::new(
                target.clone(),
                source.clone(),
                fstype.clone(),
                old_ops,
...
fn build_device_graph<'a>(ptr: &mut Device, b: &Buckets<'a>, ys: &'a HashSet<Mount>) {
...
fn main() {
    let (message_tx, rx) = mpsc::unbounded();
    ...
    let f2 = rx
        .for_each(
            move |(cmd, connections_tx): (Command, UnboundedSender<_>)| {
                let state = state.clone();
                let mut state = state.lock().unwrap();

                {
                    match cmd {
                        Command::UdevCommand(x) => update_udev(&mut state, x),
                        Command::MountCommand(x) => update_mount(&mut state, x),
                        _ => (),
                    };
                };

                let dev_list = build_device_list(&mut state.uevents);
