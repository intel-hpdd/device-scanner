...
pub struct Cache {
    entries: HashMap<String, (delay_queue::Key, HashSet<Device>)>,
...
impl Cache {
    ...
    fn insert(&mut self, host: &str, state: HashSet<Device>) {
        let key = self
            .expirations
    ...
    /// the default TTL.
    pub fn upsert(&mut self, host: &str, state: HashSet<Device>) {
    ...
    pub fn entries(&self) -> HashMap<String, HashSet<Device>> {
        self.entries
            .iter()
